...
#password {
	border: 2px solid <?php if (isset( $_COOKIE[$cookieName] )) { print "red"; } else { print "#959595"; } ?>;
	font-size: 18px;
	padding: 5px;
	width: 305px;
}
...
function redirect()
{
	...
	document.cookie = "<?=$cookieName?>=weeblylogin:"+password+"; expires=<?=date("D, d-M-Y", strtotime("+30 days"))?> 00:00:01 GMT; path=/";
...
<body style='background: #F2F2F2; text-align: center; margin: 0; padding: 0;'>
	<div id="login">
		...
		<form onsubmit="redirect();return false;">
			...
			<?php if (isset( $_COOKIE[$cookieName] )) { print "<p class='error-message'>Invalid password, please try again.</p>"; } ?>
		</form>
	</div>
</body>
</html>
